...
-- 기존 테이블 삭제 (의존성 순서 역순)
DROP TABLE IF EXISTS rwis.rdf01hh_tb CASCADE;
DROP TABLE IF EXISTS rwis.rdr01mi_tb_old CASCADE;
DROP TABLE IF EXISTS rwis.rdievent_tb CASCADE;
DROP TABLE IF EXISTS rwis.rdzgahighlist_tb CASCADE;
DROP TABLE IF EXISTS rwis.rdzgalist_tb CASCADE;
DROP TABLE IF EXISTS rwis.rditag_tb CASCADE;
DROP TABLE IF EXISTS rwis.rditagunit_tb CASCADE;
DROP TABLE IF EXISTS rwis.rdisaup_tb CASCADE;
DROP TABLE IF EXISTS rwis.rdzgacode_tb CASCADE;

-- ============================================
-- 1. RDZGACODE_TB (사업장코드)
-- ============================================
CREATE TABLE rwis.rdzgacode_tb (
...
COMMENT ON TABLE rwis.rdzgacode_tb IS '사업장코드';
COMMENT ON COLUMN rwis.rdzgacode_tb.wqe_suj_code IS 'WQE 정수장 코드';
COMMENT ON COLUMN rwis.rdzgacode_tb.rwis_suj_code IS 'RWIS 정수장 코드';
COMMENT ON COLUMN rwis.rdzgacode_tb.krm_suj_code IS 'KRM 정수장 코드';
COMMENT ON COLUMN rwis.rdzgacode_tb.suj_name IS '정수장명';
COMMENT ON COLUMN rwis.rdzgacode_tb.order_num IS '정렬순서';

-- ============================================
-- 2. RDISAUP_TB (사업장)
-- ============================================
CREATE TABLE rwis.rdisaup_tb (
...
COMMENT ON TABLE rwis.rdisaup_tb IS '사업장';
COMMENT ON COLUMN rwis.rdisaup_tb.suj_code IS '사업장코드';
COMMENT ON COLUMN rwis.rdisaup_tb.suj_name IS '사업장이름';
COMMENT ON COLUMN rwis.rdisaup_tb.bnb_code IS '본부코드';
COMMENT ON COLUMN rwis.rdisaup_tb.sms_code IS '사무소코드';
COMMENT ON COLUMN rwis.rdisaup_tb.gnj_code IS '광역지방구분 (G:광역, J:지방)';
COMMENT ON COLUMN rwis.rdisaup_tb.icus IS '주소';
COMMENT ON COLUMN rwis.rdisaup_tb.crdt IS '생성일시';

-- ============================================
-- 3. RDITAGUNIT_TB (디지털/아날로그 구분정보)
-- ============================================
CREATE TABLE rwis.rditagunit_tb (
...
COMMENT ON TABLE rwis.rditagunit_tb IS '디지털/아날로그 구분정보';
COMMENT ON COLUMN rwis.rditagunit_tb.tag_unit IS '태그타입';
COMMENT ON COLUMN rwis.rditagunit_tb.unit_desc IS '태그설명';
COMMENT ON COLUMN rwis.rditagunit_tb.wdims_dpoint IS '품질포인트';
COMMENT ON COLUMN rwis.rditagunit_tb.wdims_useyn IS 'WDIMS 사용유무';

-- ============================================
-- 4. RDITAG_TB (태그 정보 테이블)
-- ============================================
CREATE TABLE rwis.rditag_tb (
...
COMMENT ON TABLE rwis.rditag_tb IS '태그 정보 테이블';
COMMENT ON COLUMN rwis.rditag_tb.tagsn IS '태그번호';
COMMENT ON COLUMN rwis.rditag_tb.bnb_code IS '유역본부코드';
COMMENT ON COLUMN rwis.rditag_tb.sms_code IS '사무소코드';
COMMENT ON COLUMN rwis.rditag_tb.suj_code IS '사업장코드';
COMMENT ON COLUMN rwis.rditag_tb.br_code IS '변량코드';
COMMENT ON COLUMN rwis.rditag_tb.kn_code IS '기능코드';
COMMENT ON COLUMN rwis.rditag_tb.gt_code IS '계통코드';
COMMENT ON COLUMN rwis.rditag_tb.gb_code IS '구분코드(중복방지)';
COMMENT ON COLUMN rwis.rditag_tb.tag_unit IS '단위';
COMMENT ON COLUMN rwis.rditag_tb.tag_gubun IS '태그 구분 코드';
COMMENT ON COLUMN rwis.rditag_tb.rwis_use_yn IS '태그 사용여부';
COMMENT ON COLUMN rwis.rditag_tb.bonsa_tran_yn IS '본사 전송 여부';
COMMENT ON COLUMN rwis.rditag_tb.data_process IS '데이터처리방식';
COMMENT ON COLUMN rwis.rditag_tb.tag_desc IS '태그 설명';
COMMENT ON COLUMN rwis.rditag_tb.tag_alias IS '태그매핑주소';
COMMENT ON COLUMN rwis.rditag_tb.tag_full IS '유효상한값';
COMMENT ON COLUMN rwis.rditag_tb.dsp_base IS '표시하한값';
COMMENT ON COLUMN rwis.rditag_tb.dsp_full IS '표시상한값';
COMMENT ON COLUMN rwis.rditag_tb.limi_low IS '태그 하한값';
COMMENT ON COLUMN rwis.rditag_tb.limi_high IS '태그 상한값';
COMMENT ON COLUMN rwis.rditag_tb.gnj_code IS '광역지방구분코드';

-- ============================================
-- 5. RDZGALIST_TB (사업장리스트)
-- ============================================
CREATE TABLE rwis.rdzgalist_tb (
    wqe_suj_code VARCHAR(20) NOT NULL PRIMARY KEY,
...
COMMENT ON TABLE rwis.rdzgalist_tb IS '사업장리스트';
COMMENT ON COLUMN rwis.rdzgalist_tb.wqe_suj_code IS 'WQE 정수장 코드';
COMMENT ON COLUMN rwis.rdzgalist_tb.process_div IS '공정구분 (일반/후오존/전오존/UV)';
COMMENT ON COLUMN rwis.rdzgalist_tb.krm_level IS 'KRM 위기 단계 (평시/관심/주의)';
COMMENT ON COLUMN rwis.rdzgalist_tb.ga_alert_yn IS '조류경보 대상 여부';
COMMENT ON COLUMN rwis.rdzgalist_tb.ga_alert_level IS '조류경보 발령단계';
COMMENT ON COLUMN rwis.rdzgalist_tb.intake_yn IS '선택취수 가능 여부';
COMMENT ON COLUMN rwis.rdzgalist_tb.intake_location IS '선택취수 위치(표층/중층/심층)';
COMMENT ON COLUMN rwis.rdzgalist_tb.intake_depth IS '취수 수심';
COMMENT ON COLUMN rwis.rdzgalist_tb.ga_barrier_yn IS '조류 차단막 설치 여부';
COMMENT ON COLUMN rwis.rdzgalist_tb.pac_rate IS '분말활성탄 주입률';
COMMENT ON COLUMN rwis.rdzgalist_tb.floc_rate IS '응집제 주입률';
COMMENT ON COLUMN rwis.rdzgalist_tb.pre_cl_rate IS '전염소 주입률';
COMMENT ON COLUMN rwis.rdzgalist_tb.mid_cl_rate IS '중염소 주입률';

-- ============================================
-- 6. RDZGAHIGHLIST_TB (상한값리스트)
-- ============================================
CREATE TABLE rwis.rdzgahighlist_tb (
    wqe_suj_code VARCHAR(20) NOT NULL PRIMARY KEY,
    process_div VARCHAR(20),
    krm_level VARCHAR(20),
    ga_alert_yn VARCHAR(20),
...
COMMENT ON TABLE rwis.rdzgahighlist_tb IS '상한값리스트';
COMMENT ON COLUMN rwis.rdzgahighlist_tb.wqe_suj_code IS 'WQE 정수장 코드';
COMMENT ON COLUMN rwis.rdzgahighlist_tb.process_div IS '공정구분';
COMMENT ON COLUMN rwis.rdzgahighlist_tb.krm_level IS 'KRM 위기 단계';

-- ============================================
-- 7. RDR01MI_TB_OLD (01분 원시 데이터)
-- ============================================
CREATE TABLE rwis.rdr01mi_tb_old (
...
COMMENT ON TABLE rwis.rdr01mi_tb_old IS '01분 원시 데이터';
COMMENT ON COLUMN rwis.rdr01mi_tb_old.log_time IS '저장시간 (YYYYMMDDHHMM)';
COMMENT ON COLUMN rwis.rdr01mi_tb_old.tagsn IS '태그번호';
COMMENT ON COLUMN rwis.rdr01mi_tb_old.val IS '측정값';

-- ============================================
-- 8. RDF01HH_TB (시간테이블)
-- ============================================
CREATE TABLE rwis.rdf01hh_tb (
...
COMMENT ON TABLE rwis.rdf01hh_tb IS '시간테이블 (1시간 단위 집계)';
COMMENT ON COLUMN rwis.rdf01hh_tb.log_time IS '저장시간 (YYYYMMDDHH)';
COMMENT ON COLUMN rwis.rdf01hh_tb.tagsn IS '태그번호';
COMMENT ON COLUMN rwis.rdf01hh_tb.val IS '집계값';
COMMENT ON COLUMN rwis.rdf01hh_tb.fst_val IS '최초값';

-- ============================================
-- 9. RDIEVENT_TB (이벤트)
-- ============================================
CREATE TABLE rwis.rdievent_tb (
...
COMMENT ON TABLE rwis.rdievent_tb IS '이벤트';
COMMENT ON COLUMN rwis.rdievent_tb.occur_time IS '발생시간';
COMMENT ON COLUMN rwis.rdievent_tb.event_type IS '이벤트 타입';
COMMENT ON COLUMN rwis.rdievent_tb.proc_name IS '프로시저명';
COMMENT ON COLUMN rwis.rdievent_tb.proc_type IS '프로시저형식';
COMMENT ON COLUMN rwis.rdievent_tb.event_id IS '이벤트ID';
COMMENT ON COLUMN rwis.rdievent_tb.user_id IS '사용자ID';
COMMENT ON COLUMN rwis.rdievent_tb.machine_name IS '기기명';
COMMENT ON COLUMN rwis.rdievent_tb.event_descr IS '이벤트설명';
...
INSERT INTO rwis.rdzgacode_tb (wqe_suj_code, rwis_suj_code, krm_suj_code, suj_name, order_num) VALUES
...
INSERT INTO rwis.rdisaup_tb (suj_code, suj_name, bnb_code, sms_code, gnj_code, icus) VALUES
...
INSERT INTO rwis.rditagunit_tb (tag_unit, unit_desc, wdims_dpoint, wdims_useyn) VALUES
(1, 'm3/h', 2, 'Y'),
(2, 'mg/L', 3, 'Y'),
(3, 'NTU', 2, 'Y'),
(4, '℃', 1, 'Y'),
...
INSERT INTO rwis.rditag_tb (tagsn, bnb_code, sms_code, suj_code, br_code, kn_code, gt_code, gb_code, tag_unit, tag_gubun, rwis_use_yn, bonsa_tran_yn, data_process, tag_desc, tag_alias, tag_deci, tag_base, tag_full, dsp_base, dsp_full, limi_low, limi_high, gnj_code) VALUES
...
INSERT INTO rwis.rdr01mi_tb_old (log_time, tagsn, val) VALUES
-- 2026년 1월 7일 데이터
...
INSERT INTO rwis.rdf01hh_tb (log_time, tagsn, val, fst_val) VALUES
-- 2026년 1월 6일 시간별 데이터
...
INSERT INTO rwis.rdievent_tb (occur_time, event_type, proc_name, proc_type, event_id, user_id, machine_name, event_descr) VALUES
...
-- 인덱스 생성
CREATE INDEX idx_rdr01mi_log_time ON rwis.rdr01mi_tb_old(log_time);
CREATE INDEX idx_rdr01mi_tagsn ON rwis.rdr01mi_tb_old(tagsn);
CREATE INDEX idx_rdf01hh_log_time ON rwis.rdf01hh_tb(log_time);
CREATE INDEX idx_rditag_suj_code ON rwis.rditag_tb(suj_code);
CREATE INDEX idx_rditag_bnb_code ON rwis.rditag_tb(bnb_code);
CREATE INDEX idx_rdievent_occur_time ON rwis.rdievent_tb(occur_time);
CREATE INDEX idx_rdievent_event_type ON rwis.rdievent_tb(event_type);

-- 완료 메시지
DO $$
BEGIN
    RAISE NOTICE 'RWIS 테스트 스키마 및 데이터 생성 완료!';
